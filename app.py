...
try:
    ...
    st.error("데이터베이스 연결 실패. 시스템 관리자에게 문의하십시오.")
    st.stop()
...
with tab1:
    with st.form("alert_register_form"):
        ...
        with col3:
            indicator = st.selectbox("분석 지표", ["RSI", "MACD", "SMA_20", "SMA_60"])
        with col4:
            operator = st.selectbox("조건", ["<=", "<", ">=", ">"])
        with col5:
            target_value = st.number_input("목표 기준값", value=30.0, step=0.1)
...
def delete_alert(alert_id):
    try:
        ...
        st.success("해당 알람이 시스템에서 영구 삭제되었습니다.")
    except Exception as e:
        st.error(f"삭제 실패: {e}")
...
with tab2:
    ...
    if login_submit:
        if not login_email or not login_pw:
            ...
            # DB에서 이메일과 비밀번호가 일치하는 데이터 검출
            response = supabase.table("alerts_pro").select("*").eq("email", login_email).eq("password", login_pw).execute()
            records = response.data
            
            if not records:
                st.info("일치하는 알람 기록이 없거나 비밀번호가 틀렸습니다.")
            else:
                st.write(f"총 {len(records)}개의 기록이 발견되었습니다.")
